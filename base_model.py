...
class SIR_model:
    ...
    def __init__(self, S: int, I: int, R: int, N:int, beta: float, gamma: float, T:int):
        ...
        # Suseptible (S)
        self.suseptible = S
        self.suseptible_fraction = S/N

        # Infectious (I)
        self.infectious = I
        self.infectious_fraction = I/N

        # Recovered (R)
        self.recovered = R
        self.recovered_fraction = R/N

        # Contact rate (beta)
        self.contact_rate = beta
    ...
    def suseptible_dynamics(self, S_step_increment = 0.0, I_step_increment = 0.0):
        return (- self.contact_rate * (self.infectious_data[-1] + I_step_increment) 
                * (self.suseptible_data[-1] / self.total_population + S_step_increment))
    ...
    def recovered_dynamics(self, I_step_increment = 0.0):
        ...
        #return self.__total_population - self.__suseptible - self.infectious
    ...
    def Runge_Kutta_fourth_order(self, increment):
        ...
        #self.recovered_data.append(self.total_population - self.suseptible_data[-1] - self.infectious_data[-1])
    ...
    def compute(self, increment: float):
        ...
        self.recovered_data.append(float(self.total_population - self.suseptible - self.infectious))
